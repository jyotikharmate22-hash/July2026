...
(async () => {
  ...
  try {
    ...
    // Get all the form dropdowns by looking for "-- Select --" text
    ...
    console.log(`Found ${selectButtonCount} "-- Select --" buttons`);
    
    // === STEP 1: Select User Role (first dropdown) ===
    console.log('\n=== STEP 1: Select User Role ===');
    console.log('Clicking first dropdown (User Role)...');
    ...
    // === STEP 4: Select Status (second dropdown) ===
    console.log('\n=== STEP 4: Select Status ===');
    const allSelectButtons = page.locator('button').filter({ hasText: /^-- Select --$/i });
    const remainingSelectButtons = await allSelectButtons.count();
    console.log(`Remaining "-- Select --" buttons: ${remainingSelectButtons}`);
    
    console.log('Clicking second dropdown (Status)...');
    await allSelectButtons.nth(1).click();
    await page.waitForTimeout(500);
    ...
    // Get all text inputs (excluding the employee name input)
    const allInputs = page.locator('form input[type="text"]');
    const inputCount = await allInputs.count();
    console.log(`Found ${inputCount} text inputs in form`);
    
    // Username should be the second text input (after employee name)
    const usernameInput = allInputs.nth(1);
